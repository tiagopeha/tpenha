...
import { GameLogBuilder, battingLineOf } from './helpers';
...
describe('Cenário 01 — sac fly não conta AB, mas conta RBI', () => {
  ...
  g.advance('camisa-5', 3, 'home'); // reclassifica e marca
...
describe('Cenário 03 — escolha do defensor com corrida marcando', () => {
  ...
  g.pa('camisa-5', 'fieldersChoice');
  g.out('camisa-4', 2, 'forceOut');
  g.advance('camisa-3', 3, 'home');
...
describe('Cenário 04 — correção de jogada depois que o próximo rebatedor já foi anotado', () => {
  ...
  it('o estado re-deriva como se a jogada tivesse sido anotada certa desde o início', () => {
    const corrected = deriveGameState(build().events);
    ...
    expect(corrected).toEqual(deriveGameState(clean.events));
    expect(corrected.bases.first).toBe('camisa-1');
    expect(corrected.outs).toBe(1);
  });
});

describe('Cenário 05 — fim de jogo por mercy rule', () => {
  // A regra exata (diferença de corridas × entrada mínima) vem do RulesProfile
  // da categoria — questão em aberto nº 1 da spec. O core não decide o mercy:
  // registra o encerramento decidido na marcação e preserva o placar.
  const g = new GameLogBuilder();
  g.gameCreated({ home: true });
  g.halfInning(1, 'bottom');
  ...
  it('jogo encerra com endReason mercyRule e placar preservado', () => {
    const state = deriveGameState(g.events);
    expect(state.status).toBe('ended');
    expect(state.endReason).toBe('mercyRule');
